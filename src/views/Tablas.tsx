...
export function Tablas() {
  ...
  // Estado para selección de tablas
  ...
  const theme = {
    bgMain: '#050505',
    bgPanel: '#111111',
    bgTableAlt1: '#0a0a0a', // Fila oscura par
    bgTableAlt2: '#141414', // Fila oscura impar
    bgHeader1: '#1a1a1a',   // Encabezado principal
    bgHeader2: '#222222',   // Sub-encabezado
    bgHighlight: 'rgba(255, 179, 0, 0.25)', // Resaltado ámbar translúcido
    border: '#333333',
    textMain: '#cccccc',
    textAccent: '#ffb300', // Ámbar
    textDist: '#ffffff'    // Blanco brillante para distancia
  };

  // Componente interno para dibujar UNA tabla
      ...
      <div style={{ flex: 1, overflow: 'auto', background: theme.bgMain, scrollbarWidth: 'thin' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem', fontFamily: 'Share Tech Mono, monospace', textAlign: 'center', color: theme.textMain }}>
          <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
            {/* Encabezado Nivel 1 (Distancia y Título Cargas) */}
          ...
          <tbody>
            {data.map((row, idx) => {
              // Resaltar si coincide con el filtro (+- 50m)
              const highlight = filtro && Math.abs(row.dist - parseInt(filtro)) < 50;
              // Color de fondo alternado o resaltado
              ...
              return (
                <tr key={idx} style={{ background: bgColor, transition: 'background 0.2s' }}>
                  {/* Columna de Distancia (Destacada) */}
  ...
  return (
    <div style={{ padding: '15px', height: '100%', display: 'flex', flexDirection: 'column', background: theme.bgMain, color: theme.textMain }}>
      ...
      {/* ÁREA DE VISUALIZACIÓN DE TABLAS */}
