...
import { latLonToUtm, utmToLatLng, dmsToDecimal, decimalToDms } from '../logic/calculos';
...
interface DMSInputGroupProps {
    ...
    s: string;
    color: string;
    onChange: (campo: 'd' | 'm' | 's', valor: string) => void;
}
    ...
    return (
        <div className="laptop-bezel" style={{ width: '100%', height: '100%', border: 'none', display: 'flex', flexDirection: 'column', background: COLORS.bg }}>
            <div className="screen-container" style={{ display: 'flex', flexDirection: 'column', padding: '20px', alignItems: 'center', overflowY: 'auto' }}>
                ...
                {/* --- HEADER --- */}
                <header className="screen-header" style={{ width: '100%', maxWidth: '800px', marginBottom: '20px', borderBottom: `2px solid ${COLORS.border}`, justifyContent: 'space-between', display: 'flex' }}>
                    <div className="header-left" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div className="status-led online" style={{ boxShadow: `0 0 10px ${COLORS.neon}`, width: '10px', height: '10px', borderRadius: '50%', background: COLORS.neon }}></div>
                        <h1 style={{ color: COLORS.neon, textShadow: `0 0 5px ${COLORS.neon}`, margin: 0 }}>HERRAMIENTAS DE CONVERSIÓN</h1>
                    </div>
...
// ==========================================
// 1. CONVERTIDOR ANGULAR
// ==========================================
function ConvertidorAngular() {
    const [entrada, setEntrada] = useState<string>('');
    const [origen, setOrigen] = useState<UnidadAngular>('deg');

    // CALCULAMOS EL RESULTADO EN EL VUELO (Derived State) - ADIÓS useEffect!
    const calcularResultados = () => {
        const valor = parseFloat(entrada);
        if (isNaN(valor) || entrada === '') return { deg: 0, nato: 0, ruso: 0 };
...
function ConvertidorCoordenadas() {
    ...
    const calcToGeo = () => {
        ...
        const [lat, lon] = utmToLatLng(eVal, nVal, zona, true);
        setResGeo({ lat, lon });
    };
                ...
                <div style={cardStyle}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        <div>
                            ...
                            <DMSInputGroup d={geo.latD} m={geo.latM} s={geo.latS} color={COLORS.amber} onChange={(f, v) => setGeo({ ...geo, [f === 'd' ? 'latD' : f === 'm' ? 'latM' : 'latS']: v })} />
                        </div>
                        <div>
                            <span style={{ color: COLORS.amber, fontSize: '0.7rem' }}>LONGITUD (OESTE)</span>
                            <DMSInputGroup d={geo.lonD} m={geo.lonM} s={geo.lonS} color={COLORS.amber} onChange={(f, v) => setGeo({ ...geo, [f === 'd' ? 'lonD' : f === 'm' ? 'lonM' : 'lonS']: v })} />
                        </div>
...
const cardStyle: React.CSSProperties = { background: COLORS.dark, border: `1px solid ${COLORS.border}`, padding: '20px', borderRadius: '8px' };
const labelStyle: React.CSSProperties = { color: COLORS.muted, fontSize: '0.9rem', display: 'block', marginBottom: '15px', fontWeight: 'bold' };
const inputBigStyle: React.CSSProperties = { width: '100%', background: '#000', border: `3px solid ${COLORS.amber}`, color: COLORS.amber, fontSize: '2rem', fontWeight: 'bold', padding: '15px', textAlign: 'center', fontFamily: 'monospace', outline: 'none', borderRadius: '8px', boxSizing: 'border-box' };
const inputCompactStyle: React.CSSProperties = { width: '100%', background: '#000', border: `1px solid ${COLORS.amber}`, color: '#fff', fontSize: '1.2rem', padding: '10px', textAlign: 'center', fontFamily: 'monospace', boxSizing: 'border-box' };
const resultBoxStyle: React.CSSProperties = { background: `${COLORS.dark}dd`, border: `2px solid ${COLORS.neon}`, padding: '20px', borderRadius: '8px', marginTop: '20px' };
...
const BotonSelector: React.FC<BotonSelectorProps> = ({ label, sub, active, onClick }) => (
...
const FilaResultado: React.FC<FilaResultadoProps> = ({ label, valor, esOrigen }) => (
...
const DMSInputGroup: React.FC<DMSInputGroupProps> = ({ d, m, s, color, onChange }) => {
    const style: React.CSSProperties = { background: '#000', border: `1px solid ${color}`, color: color, textAlign: 'center', fontSize: '1.2rem', padding: '10px', width: '100%', fontWeight: 'bold', boxSizing: 'border-box' };
    return (
