...
import type { LogTiro, ReglajeState } from '../../types/fdc';
...
export function EditReglajeModal({ log, onClose, onSave }: EditReglajeModalProps) {
    ...
    const [editData, setEditData] = useState<ReglajeState>(() => {
        if (log.fullData?.rawReglaje) return log.fullData.rawReglaje;
        return {
            metodo: 'apreciacion',
            dir: 'right',
            val_dir: 0,
    ...
    const inputStyle: React.CSSProperties = {
        ...
        border: '1px solid #333', padding: '4px', outline: 'none',
    };
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                {/* CABECERA */}
                <div style={{
                    background: '#ffb300', color: '#000', padding: '5px 10px',
                ...
                <div style={{ padding: '15px' }}>
                    <div style={{ marginBottom: '15px', textAlign: 'center' }}>
                        <span style={{ background: '#222', color: '#00bcd4', border: '1px solid #00bcd4', padding: '2px 8px', fontSize: '0.7rem', fontWeight: 'bold', letterSpacing: '1px' }}>
                        ...
                        <>
                            <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <select id="dir" value={editData.dir} onChange={handleChange} style={inputStyle}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <input type="number" id="val_dir" value={editData.val_dir} onChange={handleChange} style={inputStyle} />
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                                <div style={{ flex: 1 }}>
                                    <label style={{ color: '#aaa', fontSize: '0.7rem' }}>ALCANCE</label>
                                    <select id="rango" value={editData.rango} onChange={handleChange} style={inputStyle}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <input type="number" id="val_rango" value={editData.val_rango} onChange={handleChange} style={inputStyle} />
                                </div>
                            </div>
                        </>
                    ) : (
                        <>
                            <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <input type="number" id="imp_az" value={editData.imp_az} onChange={handleChange} style={inputStyle} />
                                </div>
                                <div style={{ flex: 0.6 }}>
                                    <label style={{ color: '#aaa', fontSize: '0.7rem' }}>UNIT</label>
                                    <select id="imp_unit" value={editData.imp_unit} onChange={handleChange} style={inputStyle}>
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                <input type="number" id="imp_dist" value={editData.imp_dist} onChange={handleChange} style={inputStyle} />
                            </div>
                        </>
                    )}

                    <button
                        onClick={() => onSave(log.id, editData)}
                        onMouseOver={e => (e.currentTarget.style.background = '#4a0000')}
                        onMouseOut={e => (e.currentTarget.style.background = '#330000')}
                        style={{ width: '100%', background: '#330000', color: '#ff4444', border: '1px solid #ff4444', padding: '10px', cursor: 'pointer', fontWeight: 'bold', letterSpacing: '1px', transition: 'background 0.2s' }}
                    >
